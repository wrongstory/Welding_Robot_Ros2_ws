...
: node_(node), pose_cache_(node.get()), e_theta_filter_(5)
{
  // ── Common control parameters ──
  control_rate_hz_ = safeParam(node_, "control_rate_hz", 50.0);

  // ── Translate-specific parameters ──
  Kp_heading_            = safeParam(node_, "translate_Kp_heading", 1.0);
  Kd_heading_            = safeParam(node_, "translate_Kd_heading", 0.3);
  K_stanley_             = safeParam(node_, "translate_K_stanley", 2.0);
  K_soft_                = safeParam(node_, "translate_K_soft", 1.0);
  max_omega_             = safeParam(node_, "translate_max_omega", 1.0);
  double alpha_max       = safeParam(node_, "translate_alpha_max", 0.5);
  alpha_max_             = alpha_max;
  double heading_threshold_deg = safeParam(node_, "translate_heading_threshold_deg", 45.0);
  heading_threshold_     = heading_threshold_deg * M_PI / 180.0;
  max_lateral_offset_    = safeParam(node_, "translate_max_lateral_offset", 1.0);
  int heading_filter_window = safeParam(node_, "translate_heading_filter_window", 5);
  ...
  goal_reach_threshold_      = safeParam(node_, "translate_goal_reach_threshold", 0.05);
  arrival_tolerance_         = safeParam(node_, "translate_arrival_tolerance", 0.03);
  min_vx_                    = safeParam(node_, "translate_min_vx", 0.02);
  behind_start_speed_        = safeParam(node_, "translate_behind_start_speed", 0.2);
  // behind-start 정책: false=시작점까지 behind_start_speed_ 고정 접근(기존), true=시작점 뒤부터 가속
  behind_start_accelerate_   = safeParam(node_, "translate_behind_start_accelerate", false);
  behind_start_kick_         = safeParam(node_, "translate_behind_start_kick", 0.1);
  max_timeout_sec_           = safeParam(node_, "translate_max_timeout_sec", 60.0);
  localization_timeout_sec_  = safeParam(node_, "translate_localization_timeout_sec", 2.0);
  position_jump_threshold_   = safeParam(node_, "translate_position_jump_threshold", 0.3);
  enable_localization_watchdog_ = safeParam(node_, "translate_enable_localization_watchdog", true);
  walk_accel_limit_          = safeParam(node_, "translate_walk_accel_limit", 0.5);
  walk_decel_limit_          = safeParam(node_, "translate_walk_decel_limit", 1.0);

  // 도착 2단 감속 캡 + 부드러운 정지 (T-AMR rosbag 측정 기반)
  approach_gain_             = safeParam(node_, "translate_approach_gain", 1.5);
  stop_speed_                = safeParam(node_, "translate_stop_speed", 0.2);
  stop_decel_                = safeParam(node_, "translate_stop_decel", 0.1);
  // mpc_light 감속(Issue: 고속 정지 매끄러움) — jerk-제한 S-curve v_cap + 출력 rate-limit
  jerk_max_                  = safeParam(node_, "translate_jerk_max", 1.0);
  a_decel_max_               = safeParam(node_, "translate_a_decel_max", 0.4);
  should_stop_speed_         = safeParam(node_, "translate_should_stop_speed", 0.1);
  ...
  action_server_ = rclcpp_action::create_server<Translate>(
    node_, "amr_translate_action",
    std::bind(&TranslateActionServer::handle_goal, this, _1, _2),
    std::bind(&TranslateActionServer::handle_cancel, this, _1),
    std::bind(&TranslateActionServer::handle_accepted, this, _1));
  ...
  update_endpoint_srv_ = node_->create_service<amr_interfaces::srv::UpdateTranslateEndpoint>(
    ...
    std::bind(&TranslateActionServer::onUpdateEndpoint, this,
  ...
  RCLCPP_INFO(node_->get_logger(),
    "TranslateActionServer (2WD) initialized (pos=/robot_pose, heading=imu/data fused, loc_watchdog=%s, safety=ON)",
...
rclcpp_action::GoalResponse TranslateActionServer::handle_goal(
  ...
  std::shared_ptr<const Translate::Goal> goal)
{
  ...
  if (dist < 1e-6) {
    RCLCPP_WARN(node_->get_logger(), "Translate rejected: start == end");
    return rclcpp_action::GoalResponse::REJECT;
  }
  if (goal->max_linear_speed <= 0.0) {
    RCLCPP_WARN(node_->get_logger(), "Translate rejected: max_linear_speed <= 0");
    return rclcpp_action::GoalResponse::REJECT;
  }
  if (goal->acceleration <= 0.0) {
    RCLCPP_WARN(node_->get_logger(), "Translate rejected: acceleration <= 0");
    return rclcpp_action::GoalResponse::REJECT;
  }

  // Mutual exclusion (전 서버 공유)
  auto expected = ActiveAction::NONE;
  if (!g_active_action.compare_exchange_strong(expected, ActiveAction::TRANSLATE)) {
    RCLCPP_WARN(node_->get_logger(), "Translate rejected: %s is running", to_string(expected));
    return rclcpp_action::GoalResponse::REJECT;
  }

  RCLCPP_INFO(node_->get_logger(),
    "Translate goal accepted: (%.2f,%.2f)->(%.2f,%.2f), dist=%.3f m, v_max=%.2f m/s",
    goal->start_x, goal->start_y, goal->end_x, goal->end_y,
    dist, goal->max_linear_speed);
  return rclcpp_action::GoalResponse::ACCEPT_AND_EXECUTE;
}

rclcpp_action::CancelResponse TranslateActionServer::handle_cancel(
  const std::shared_ptr<GoalHandleTranslate> /*goal_handle*/)
{
  RCLCPP_INFO(node_->get_logger(), "Translate cancel requested");
  return rclcpp_action::CancelResponse::ACCEPT;
}

void TranslateActionServer::handle_accepted(
  const std::shared_ptr<GoalHandleTranslate> goal_handle)
{
  try {
...
void TranslateActionServer::execute(
  const std::shared_ptr<GoalHandleTranslate> goal_handle)
{
  ...
  Kp_heading_ = safeParam(node_, "translate_Kp_heading", Kp_heading_);
  Kd_heading_ = safeParam(node_, "translate_Kd_heading", Kd_heading_);
  K_stanley_  = safeParam(node_, "translate_K_stanley",  K_stanley_);
  K_soft_     = safeParam(node_, "translate_K_soft",     K_soft_);
  max_omega_  = safeParam(node_, "translate_max_omega",  max_omega_);
  alpha_max_  = safeParam(node_, "translate_alpha_max",  alpha_max_);
  ...
  RCLCPP_INFO(node_->get_logger(),
    "Translate execute: path_angle=%.1f deg, target_dist=%.3f m",
  ...
  //   중복이던 watchdog_->poseReceived() 사전검사 제거(watchdog 은 별도 pose_sub_ 없이
  //   execute 루프의 updatePose 로만 갱신).
  if (!imu_received_) {
    RCLCPP_ERROR(node_->get_logger(), "IMU data not received, aborting translate");
  ...
  if (!lookupRobotPose(pose_cache_, robot_x, robot_y, robot_yaw, node_->get_logger(), node_->get_clock())) {
    RCLCPP_ERROR(node_->get_logger(),
      "/robot_pose not available (stale/unreceived), aborting translate");
    finish_abort(-3, 0.0, 0.0, 0.0, start_time);
    return;
  }

  // ── IMU↔map yaw 오프셋 1회 등록 (heading 고속 피드백용) ──
  // 이후 heading 오차는 fuseImuYawToMap(imu_yaw, imu_yaw_offset) - theta_path_ 로 계산.
  ...
  // ── Initial pose validation (inline validateInitialPose) ──
  {
    ...
    const double e_th_ini = normalizeAngle(robot_yaw - theta_path_);

    // 시작 시점에 이미 진행축상 목표에 도달(도착 판정과 동일 기준 사용)이면
    // 에러가 아니라 성공으로 종료한다. (도착 판정: line ~416, proj >= target - tol)
    if (proj_ini >= target_distance_ - arrival_tolerance_) {
      RCLCPP_INFO(node_->get_logger(),
        "Translate: already at goal at start (proj=%.3f >= %.3f m), success",
        proj_ini, target_distance_ - arrival_tolerance_);
      publishCmdVel(cmd_vel_pub_, 0.0, 0.0);
      watchdog_->setCurrentSpeed(0.0);
      {
        nav_msgs::msg::Path empty;
      ...
      result->status              = 0;
      result->actual_distance     = proj_ini;
      result->final_lateral_error = e_d_ini;
      result->final_heading_error = e_th_ini * 180.0 / M_PI;
      result->elapsed_time        = (node_->now() - start_time).seconds();
    ...
    if (std::fabs(e_th_ini) > heading_threshold_) {
      RCLCPP_WARN(node_->get_logger(),
        "Translate initial pose validation failed: heading error=%.1f deg > %.1f deg",
    ...
    if (std::fabs(e_d_ini) > max_lateral_offset_) {
      RCLCPP_WARN(node_->get_logger(),
        "Translate initial pose validation failed: lateral offset=%.3f m > %.3f m",
  ...
  double prev_cmd_ax = 0.0;  // 출력 가속도 상태(jerk-제한 rate-limit용, Issue 8/mpc_light)
  ...
  amr_motion_control::TrapezoidalProfile profile(
    target_distance_, goal->max_linear_speed, goal->acceleration, exit_speed);
  ...
  // (전치하지 않고 단순 속도캡만 풀면 projection=0 경계에서 프로파일 속도가 0으로 떨어져
  //  불연속이 생기므로, 거리 전치로 경계 연속성을 보장한다.)
  ...
  while (rclcpp::ok() && !reached) {
    ...
    if (goal_handle->is_canceling()) {
      RCLCPP_INFO(node_->get_logger(), "Translate cancelled at dist=%.3f m", last_projection);
    ...
    if ((node_->now() - start_time).seconds() > max_timeout_sec_) {
      RCLCPP_WARN(node_->get_logger(),
        "Translate global timeout (%.1f s), dist=%.3f m", max_timeout_sec_, last_projection);
    ...
    if (!approach_offset_init) {
      ...
      if (behind_start_accelerate_ && projection < 0.0) {
        ...
        profile = amr_motion_control::TrapezoidalProfile(
          target_distance_ + approach_offset, goal->max_linear_speed,
          goal->acceleration, exit_speed);
        RCLCPP_INFO(node_->get_logger(),
          "Translate behind-start ACCELERATE: approach_offset=%.3f m "
    ...
    // ── 시간 기반 가속 v = a·t (accelerate 모드, Issue 8/10 fix) ──
    ...
    if (!behind_start_accelerate_) {
      const double behind_cap =
          std::min(behind_start_speed_, static_cast<double>(goal->max_linear_speed));
      if (projection < 0.0) {
    ...
    // ── 하이브리드 감속 캡: FAR=jerk-제한 S-curve(매끄러운 고속 감속) + NEAR=위치 P 피드백(정확 수렴) ──
    // v_cap = min( sqrt(2·a_decel_max·remaining)  [S-curve, 고속 매끄러움],
    //              approach_gain·remaining        [위치 P 피드백, remaining→0 으로 목표 정확 수렴] )
    // 교차점(~0.36m) 기준 원거리엔 S-curve, 근거리엔 위치-P 가 지배 → 자연 전환(MPC→PID).
    // 위치-P 가 measured projection 으로 목표에 정확 수렴(stall/overshoot 없음 — 이전 mpc_light
    // d_jerk 가 tol 보다 커 0.08m 못미쳐 stall 하던 버그 해소). 출력 rate-limit(아래)이 jerk 제한.
    if (projection >= 0.0) {
      const double v_far  = std::sqrt(2.0 * a_decel_max_ * remaining);  // S-curve
      const double v_near = approach_gain_ * remaining;                 // 위치 P 피드백(PID 의 P)
    ...
    // Arrival condition — should_stop 게이트(mpc_light): 위치 도달 + 저속일 때만 확정.
    // 별도 blocking ramp 루프 제거 → 정지 ramp 중에도 watchdog/safety/cancel 계속 수행.
    //  - exit_speed>0: 연속성 종료(즉시 break).
    //  - exit_speed==0: 위치 도달 후에도 |prev_cmd_vx|>=should_stop_speed_ 면 vx_profile=0 주입,
    //    아래 출력 rate-limit 가 jerk-제한 ramp-to-0 → 저속 도달 시 확정.
    ...
    if (pos_reached) {
      if (goal->exit_speed > 0.0) {
        reached = true;
        RCLCPP_INFO(node_->get_logger(),
          "Translate reached with velocity continuity (exit_speed=%.3f)", goal->exit_speed);
        break;
      }
      ...
      vx_profile = 0.0;   // 위치 도달·저속 미달 → 0 주입, rate-limit 가 매끄럽게 정지
    }
    ...
    // heading error (filtered) — 고속 IMU yaw 를 map-frame 으로 융합해 사용
    //   (위치/CTE 는 robot_yaw(pose) 가 아닌 robot_x/robot_y 만 쓰므로 영향 없음).
    const double robot_yaw_ctrl = fuseImuYawToMap(last_yaw_rad_.load(), imu_yaw_offset);
    const double e_theta_raw = normalizeAngle(robot_yaw_ctrl - theta_path_);
    ...
    // ── 출력 가속도 rate-limit (2상태 jerk-제한, mpc_light) ──
    // prev_cmd_vx(명령속도)+prev_cmd_ax(명령가속도) 2상태. 목표가속도 a_des 를 [-a_decel_max_,
    // +walk_accel_limit_] 클램프 후 jerk_max_·dt 로 변화율 제한 → 가속도 연속(기존 step-function
    // 가속도, jerk~50 m/s³ 제거; 기존 감속분기가 acc_step 오용해 슬루 0.5 였던 버그도 해소).
    {
      const double a_min = -a_decel_max_;        // 감속 한계
      const double a_max =  walk_accel_limit_;   // 가속 한계
      double a_des = (vx_profile - prev_cmd_vx) / dt;
      a_des = std::clamp(a_des, a_min, a_max);
      const double j_step = jerk_max_ * dt;      // 가속도 변화율(jerk) 한계
      double a_cmd = std::clamp(a_des, prev_cmd_ax - j_step, prev_cmd_ax + j_step);
      prev_cmd_vx += a_cmd * dt;
      if (vx_profile <= 0.0 && prev_cmd_vx < 0.0) { prev_cmd_vx = 0.0; a_cmd = 0.0; }  // 0 이하 클램프
      prev_cmd_ax = a_cmd;
    }
    double cmd_vx = prev_cmd_vx;

    publishCmdVel(cmd_vel_pub_, cmd_vx, omega);

    // Debug log (throttled ~5Hz at 50Hz rate)
    static int dbg_cnt = 0;
    if (++dbg_cnt % 10 == 0) {
      RCLCPP_INFO(node_->get_logger(),
        "[Translate] vx=%.3f omega=%.4f proj=%.3fm lat=%.3fm hdg=%.1f° e_theta=%.1f° cte=%.1f°",
        cmd_vx, omega,
        projection, e_d,
  ...
  const double final_e_th = normalizeAngle(robot_yaw - theta_path_);

  RCLCPP_INFO(node_->get_logger(),
    "Translate complete: dist=%.3f/%.3f m, lat_err=%.4f m, head_err=%.2f deg",
...
void TranslateActionServer::onUpdateEndpoint(
  ...
  std::shared_ptr<amr_interfaces::srv::UpdateTranslateEndpoint::Response> res)
{
  ...
  RCLCPP_INFO(node_->get_logger(), "Endpoint update: (%.3f, %.3f) has_next=%d",
