#ifndef AMR_MOTION_CONTROL_2WD__TRANSLATE_ACTION_SERVER_HPP_
#define AMR_MOTION_CONTROL_2WD__TRANSLATE_ACTION_SERVER_HPP_
...
private:
  ...
  // /robot_pose 캐시 (map→base_link pose 소스, 신선도 판정 포함)
  ...
  // Safety (speed limit + status via SafetySubscriber)
  ...
  //  - 2단(근거리, v<stop_speed_): v_cap = sqrt(2·stop_decel_·remaining) (완만 정속감속 a=stop_decel_,
  //    도착점 0 수렴). 전환 잔여거리 = stop_speed_^2 / (2·stop_decel_).
  //  - 도착 시 ramp-to-0 도 stop_decel_ 로(완만) → 완전정지 jerk 최소.
  ...
  double jerk_max_{1.0};                // m/s³ 출력 가속도 변화율 상한
  double a_decel_max_{0.4};             // m/s² 감속 한계(v_cap 역산 + rate-limit a_min 공용)
  double should_stop_speed_{0.1};       // m/s 도착 확정 속도 게이트
...
#endif  // AMR_MOTION_CONTROL_2WD__TRANSLATE_ACTION_SERVER_HPP_
