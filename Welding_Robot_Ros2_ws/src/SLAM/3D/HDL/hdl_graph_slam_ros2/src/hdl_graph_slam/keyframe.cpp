...
KeyFrame::KeyFrame(const std::string& directory, g2o::HyperGraph* graph) : stamp(0, 0, RCL_ROS_TIME), odom(Eigen::Isometry3d::Identity()), accum_distance(-1), cloud(nullptr), node(nullptr) {
...
void KeyFrame::save(const std::string& directory) {
  ...
  uint32_t nsec = stamp.nanoseconds() % 1000000000UL;
  ofs << "stamp " << sec << " " << nsec << "\n";
...
bool KeyFrame::load(const std::string& directory, g2o::HyperGraph* graph) {
  ...
  while(!ifs.eof()) {
    ...
    if(token == "stamp") {
      ifs >> stamp_sec >> stamp_nsec;
      stamp = rclcpp::Time(stamp_sec, stamp_nsec, RCL_ROS_TIME);
