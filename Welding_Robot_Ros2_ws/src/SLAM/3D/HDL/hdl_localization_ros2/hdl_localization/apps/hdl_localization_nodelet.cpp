...
private:
  pcl::Registration<PointT, PointT>::Ptr create_registration() {
    if(reg_method == "NDT_OMP") {
      ...
      std::shared_ptr<pclomp::NormalDistributionsTransform<PointT, PointT>> ndt(new pclomp::NormalDistributionsTransform<PointT, PointT>());
...
private:
  ...
  void points_callback(const sensor_msgs::msg::PointCloud2::ConstSharedPtr points_msg) {
    ...
    if (enable_robot_odometry_prediction && last_correction_time != rclcpp::Time(0)) {
      ...
      } else if(tf_buffer->canTransform(odom_child_frame_id, last_correction_time, odom_child_frame_id, rclcpp::Time(0), robot_odom_frame_id, rclcpp::Duration(std::chrono::milliseconds(0)))) {
        odom_delta = tf_buffer->lookupTransform(odom_child_frame_id, last_correction_time, odom_child_frame_id, rclcpp::Time(0), robot_odom_frame_id, rclcpp::Duration(std::chrono::milliseconds(0)));
      }
  ...
  void publish_odometry(const rclcpp::Time& stamp, const Eigen::Matrix4f& pose) {
    ...
    if(send_tf_transforms) {
      if(tf_buffer->canTransform(robot_odom_frame_id, odom_child_frame_id, rclcpp::Time(0))) {
        ...
        geometry_msgs::msg::TransformStamped frame_wrt_odom = tf_buffer->lookupTransform(robot_odom_frame_id, odom_child_frame_id, rclcpp::Time(0), rclcpp::Duration(std::chrono::milliseconds(100)));
