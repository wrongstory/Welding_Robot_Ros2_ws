...
private:
    ...
    rclcpp::Publisher<livox_lio_sam::msg::CloudInfo>::SharedPtr pubLaserCloudInfo;
    ...
    // Reset service for localization mode
    rclcpp::Service<livox_lio_sam::srv::Reset>::SharedPtr srvReset;
...
public:
            ...
            ParamServer("lio_sam_imageProjection", options), deskewFlag(0)
    {
        ...
        pubLaserCloudInfo = create_publisher<livox_lio_sam::msg::CloudInfo>(
            "lio_sam/deskew/cloud_info", qos);

        // Reset service for clearing queues (used by localization mode initial pose)
        srvReset = create_service<livox_lio_sam::srv::Reset>(
            "lio_sam/reset",
            std::bind(&ImageProjection::resetServiceCallback, this, std::placeholders::_1, std::placeholders::_2));
    ...
    void resetServiceCallback(
        const std::shared_ptr<livox_lio_sam::srv::Reset::Request> req,
        std::shared_ptr<livox_lio_sam::srv::Reset::Response> res)
    {
        (void)req;  // unused

        RCLCPP_INFO(get_logger(), "Reset service called - clearing IMU and Odom queues");
        ...
        RCLCPP_INFO(get_logger(), "Reset complete - queues cleared");
        res->success = true;
    }
    ...
    bool cachePointCloud(const sensor_msgs::msg::PointCloud2::SharedPtr& laserCloudMsg)
    {
        ...
        {
            // Convert Livox to Velodyne format
            ...
            // Get the first point timestamp as reference (in nanoseconds)
            double firstPointTime = tmpLivoxCloudIn->points[0].timestamp;
            ...
            for (size_t i = 0; i < tmpLivoxCloudIn->size(); i++)
            {
                ...
                dst.z = src.z;
                dst.intensity = src.intensity;
                dst.ring = src.line;  // Livox uses 'line' instead of 'ring'
                // Livox timestamp is in nanoseconds, convert to relative time in seconds
                dst.time = (src.timestamp - firstPointTime) * 1e-9;
            }
        }
        else
        {
        ...
        if (ringFlag == 0)
        {
            ...
            for (int i = 0; i < (int)currentCloudMsg.fields.size(); ++i)
            {
                if (currentCloudMsg.fields[i].name == "ring" ||
                    (sensor == SensorType::LIVOX && currentCloudMsg.fields[i].name == "line"))
                {
                    ringFlag = 1;
                    break;
                }
        ...
        if (deskewFlag == 0)
        {
            ...
            for (auto &field : currentCloudMsg.fields)
            {
                if (field.name == "time" || field.name == "t" ||
                    (sensor == SensorType::LIVOX && field.name == "timestamp"))
                {
                    deskewFlag = 1;
                    break;
                }
    ...
    bool deskewInfo()
    {
        ...
        double imuFrontTime = stamp2Sec(imuQueue.front().header.stamp);
        double imuBackTime = stamp2Sec(imuQueue.back().header.stamp);

        if (imuFrontTime > timeScanCur)
        {
            RCLCPP_INFO(get_logger(), "Waiting for IMU data ... (IMU front %.6f > scan start %.6f)", imuFrontTime, timeScanCur);
            return false;
        }

        if (imuBackTime < timeScanEnd)
        {
            RCLCPP_INFO(get_logger(), "Waiting for IMU data ... (IMU back %.6f < scan end %.6f, diff: %.6f)",
                        imuBackTime, timeScanEnd, timeScanEnd - imuBackTime);
            return false;
        }
