...
public:
            ...
            ParamServer("lio_sam_imu_preintegration", options)
    {
        ...
        srvReset = create_service<livox_lio_sam::srv::Reset>(
            "lio_sam/reset_imu_preintegration",
            std::bind(&IMUPreintegration::resetServiceCallback, this, std::placeholders::_1, std::placeholders::_2));
    }
        ...
        std::shared_ptr<livox_lio_sam::srv::Reset::Response> res)
    {
        ...
        RCLCPP_INFO(get_logger(), "Reset service called - resetting IMU preintegration");
        ...
        // Reset integrators
        imuIntegratorImu_->resetIntegration();
        imuIntegratorOpt_->resetIntegration();

        // Reset key
        key = 1;

        RCLCPP_INFO(get_logger(), "Reset complete - IMU preintegration reinitialized");
        res->success = true;
    }
